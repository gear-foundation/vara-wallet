...
- **Transparent retry on transient transport errors** (issue #60, PR #64). `getApi()` retries the connect path up to 3 attempts when the underlying failure is `TRANSPORT_ERROR` with `reason ∈ {timeout, ws_close_abnormal}`. Permanent reasons (`dns_failure`, `tls_failure`, `protocol_mismatch`, `connection_refused`, `unreachable`, `unknown`) still fail fast — retrying them just wastes wallclock. Backoff `[0, 250, 1000]ms` with ±50% jitter prevents thundering-herd when N agents fail in lockstep against the same flaky endpoint. `--verbose` surfaces each retry boundary: `[verbose] retry 1/2 after transport timeout (125ms backoff)`. Opt-out: set `VARA_NO_RETRY` to any of `1`, `true`, `yes`, or `on` (case-insensitive) for strict single-attempt semantics — recommended in CI where per-command wallclock matters (a flaky endpoint can otherwise inflate suite time by ~3×). Composes with the existing metadata-cache retry branch: transport retry alone is ~32s worst-case (3 attempts × 10s timeout + jittered backoffs); both layers together stretch to ~62s when the metadata cache is also poisoned. Median is unchanged at ~0.5s on a warm cache. Callers wrapping `getApi()` in an outer deadline should size against ~62s to cover the composed case, not ~10s. Renders the agent-side retry-loop workaround in `gear-foundation/vara-agent-network` PR #37 (already merged) redundant — downstream agent skills can drop their custom retry once they pick up this version.

### Documentation

- **`TRANSPORT_ERROR` reference in README + AGENTS** (PR #63). Documents the structured shape (`reason` taxonomy + `endpoint` / `host` / `cause` at top level — `formatError` spreads `CliError.meta`, no envelope), the retry-vs-fail-fast switch pattern matching the 0.19.0 auto-retry policy, and a migration warning for scripts still grepping the retired `CONNECTION_TIMEOUT` / `TIMEOUT` / `CONNECTION_FAILED` codes. Fixed a `meta.programMessage` → top-level `.programMessage` bug in the `PROGRAM_ERROR` row of AGENTS.md that pre-dated 0.16.0's flattening.
- **`SKILL.md` refresh for the 0.18.0 surface** (PR #67). Reframed IDL Resolution around embedded `sails:idl` as the default — `idl import` is scoped to the narrow case of contracts without an embedded section, dropping the v1/v2 framing. Documents the enveloped section format (1-byte version + 1-byte flags + payload) the 0.18.0 extractor reads, with raw UTF-8 fallback for older programs. Added `metadata list` / `metadata clear` to the Read table alongside the `idl` family. Stripped pre-0.18 fallback paragraphs and `(0.16+)` / `(0.17+)` qualifiers from sections describing the shipped surface; migration callouts that help agents update scripts pulled from older tutorials are retained.
