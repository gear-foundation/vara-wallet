...
type VftGlobalOptions = AccountOptions & TokenResolveOptions;

function resolveVftToken(input: string, opts: TokenResolveOptions): ResolvedToken {
...
export function registerVftCommand(program: Command): void {
  ...
  vft
    ...
    .argument('<tokenProgram>', 'VFT program ID (0x...) or built-in token alias')
    ...
    .action(async (tokenProgram: string, options: { idl?: string }) => {
      ...
      const token = resolveVftToken(tokenProgram, opts);
      const api = await getApi(opts.ws);

      const sails = await loadSails(api, {
        programId: token.address,
        idl: options.idl,
  ...
  vft
    ...
    .action(async (tokenProgram: string, account: string | undefined, options: { idl?: string }) => {
      const opts = program.optsWithGlobals() as VftGlobalOptions;
      const token = resolveVftToken(tokenProgram, opts);
      const api = await getApi(opts.ws);
      ...
      verbose(`Querying VFT balance for ${address} on ${token.address}`);
  ...
  vft
    ...
    .action(async (tokenProgram: string, owner: string, spender: string, options: { idl?: string }) => {
      ...
      verbose(`Querying allowance for owner=${owner} spender=${spender} on ${token.address}`);
